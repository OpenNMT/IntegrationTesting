...
ssh -o "StrictHostKeyChecking no" -i $KEY ubuntu@$HOSTNAME 'export DATE=$(date "+%Y-%m-%d-%H-%M");cd IntegrationTesting;bash setup.sh;sudo nvidia-docker exec -i opennmt bash -c "$(cat run.sh)" | tee "logs/call_log_$DATE.txt";cat "logs/call_log_$DATE.txt" | python graph.py; git add "logs/call_log_$DATE.txt"; git commit -a -m .; git push origin master;exit'
